...
public class ClassInfo extends AppCompatActivity {
	...
	private void initializeVariables() {
		...
		try {
			adminState = new FirebaseUtils.AdminState(classCode,
				new in.edu.jaduniv.classroom.interfaces.ValueEventListener() {
					@Override
	...
	private void updateAdminStatus(boolean isAdmin) {
		if (isAdmin) {
			...
			joinRequests.clear();
			FirebaseUtils.getDatabaseReference().child("classes").child(classCode).child("join_req")
				.addChildEventListener(new ChildEventListener() {
					...
					public void onChildRemoved(@NonNull DataSnapshot dataSnapshot) {
						...
						joinRequestKeys.remove(dataSnapshot.getKey());
						joinRequestAdapter.notifyDataSetChanged();
	...
	private void loadParticipants() {
		FirebaseUtils.getDatabaseReference().child("classes").child(classCode).child("participants")
			.orderByChild("name").addChildEventListener(new ChildEventListener() {
			@Override
			public void onChildAdded(@NonNull DataSnapshot dataSnapshot, String s) {
			...
			@Override
			public void onChildChanged(@NonNull DataSnapshot dataSnapshot, String s) {
				participants.set(participantsKey.indexOf(dataSnapshot.getKey()),
					dataSnapshot.getValue(Participant.class));
				participantAdapter.notifyDataSetChanged();
			}
