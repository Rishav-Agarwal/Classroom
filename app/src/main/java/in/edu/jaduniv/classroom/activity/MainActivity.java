...
public class MainActivity extends AppCompatActivity {
	...
	public void initiateVariables() {
		...
		FirebaseUtils.initialize(getApplicationContext());
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == RC_SIGN_IN) {
			if (resultCode == RESULT_CANCELED) {
				...
				referenceUsers.child(CurrentUser.getInstance().getPhone()).updateChildren(userMap)
		...
		if (requestCode == LoginInfoActivity.RC_LOGIN_INFO) {
			...
			startActivityForResult(
				AuthUI.getInstance().createSignInIntentBuilder().setIsSmartLockEnabled(true)
	...
	public void loadNavigationHeader() {
		GlideApp.with(this).load("https://atgbcentral.com/data/out/193/5690319-material-wallpaper.png")
			.diskCacheStrategy(DiskCacheStrategy.ALL).into(ivNavHeaderBackground);
		//When connected with firebase, load user data into navigation header
		referenceUsers.child(CurrentUser.getInstance().getPhone())
	...
	public void loadClasses() {
		...
		referenceUsers.child(CurrentUser.getInstance().getPhone()).child("classes")
			.addChildEventListener(new ChildEventListener() {
				@Override
				public void onChildAdded(@NonNull DataSnapshot dataSnapshot, String s) {
					...
					referenceClasses.child(extractedClassId).addChildEventListener(new ChildEventListener() {
						...
						public void onChildAdded(@NonNull DataSnapshot dataSnapshot, String s) {
							if (dataSnapshot.getKey().equals("name")) {
								...
								MenuItem newItem = classes
									.add(R.id.nav_grp_classes, classFragmentTag.size(), Menu.NONE,
										extractedClass[0]);
								...
								if (classFragmentTag.size() == 1) {
									...
									loadDefaultFragment();          //Loads a default page when user hasn't joined
									// any class
								}
							}
						}
	...
	private void setUpNavigationView() {
		...
		ActionBarDrawerToggle actionBarDrawerToggle = new ActionBarDrawerToggle(this, drawer, toolbar,
			...
			public void onDrawerClosed(View drawerView) {
				// Code here will be triggered once the drawer closes as we don't want anything to happen
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		if (item.getItemId() == R.id.menu_new_class) {
			...
			// Two options for entering a class
			final String[] joinOrCreate = {"Join a class", "Create a class"};
			...
			AlertDialog.Builder joinCreateBuilder = new AlertDialog.Builder(this)
				...
				.setAdapter(new ArrayAdapter<>(getApplicationContext(),
					...
					joinOrCreate), new DialogInterface.OnClickListener() {

					@Override
					public void onClick(DialogInterface dialog, int which) {
						if (which == 0) {
							// If join class requested, move to new activity
	...
	/**
	 * Checks is user is connected to the internet
	 *
	 * @return A boolean specifying if user is connected to the internet
	 */
